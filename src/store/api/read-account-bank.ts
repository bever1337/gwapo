...
export const injectedApi = api.injectEndpoints({
  endpoints(build) {
    return {
      readAccountBank: build.query<
        ReadAccountBankResult,
        ReadAccountBankArguments
      >({
        extraOptions: {
...
export const selectAccountBankWithTabs = createSelector(
  selectReadAccountBank,
  (queryResult) => {
    const bankTabs = [];
    for (
      let bankTabIndex = 0;
      ...
      bankTabs.push(
        queryResult.data!.slice(bankTabIndex, bankTabIndex + ITEMS_PER_TAB)
      );
    }
    return bankTabs;
  }
